#include "ViewerQGL.h"
#include <CGAL/gl.h>
#include "Scene_draw_QGLinterface.h"
#include <QMouseEvent>
#include <QKeyEvent>
...
public:
  ...
  void draw_aux(bool with_names, ViewerQGL*);
};

ViewerQGL::ViewerQGL(QWidget* parent, bool antialiasing)
  : ViewerQGL_interface(parent)
{
  //polyhedron_ptr = NULL;
  d = new ViewerQGL_impl;
  d->scene = 0;
...
void ViewerQGL::setScene(Scene_draw_interface* scene)
{
  d->scene = scene;
}

bool ViewerQGL::antiAliasing() const
{
  return d->antialiasing; 
}

void ViewerQGL::setAntiAliasing(bool b)
{
  d->antialiasing = b;
  updateGL();
}

void ViewerQGL::setTwoSides(bool b)
{
  d->twosides = b;
  updateGL();
}

bool ViewerQGL::inFastDrawing() const {
...
void ViewerQGL::mousePressEvent(QMouseEvent* event)
{
...
void ViewerQGL::keyPressEvent(QKeyEvent* e)
{
  if(!e->modifiers()) {
    if(e->key() == Qt::Key_T) {
...
void ViewerQGL::turnCameraBy180Degres() {
...
void ViewerQGL_impl::draw_aux(bool with_names, ViewerQGL* viewer)
{
  if(scene == 0)
    return;
...
void ViewerQGL::drawWithNames()
{
  QGLViewer::draw();
  d->draw_aux(true, this);
}

void ViewerQGL::postSelection(const QPoint& pixel)
{
  bool found = false;
...
bool ViewerQGL_interface::readFrame(QString s, qglviewer::Frame& frame)
{
...
QString ViewerQGL_interface::dumpFrame(const qglviewer::Frame& frame) {
...
bool ViewerQGL::moveCameraToCoordinates(QString s, float animation_duration) {
...
QString ViewerQGL::dumpCameraCoordinates()
{
  if(camera()->frame()) {
...
void ViewerQGL::pickMatrix(GLdouble x, GLdouble y, GLdouble width, GLdouble height,
GLint viewport[4])
{
 //GLfloat m[16];
 GLfloat sx, sy;
...
void ViewerQGL::beginSelection(const QPoint &point)
{
...
void ViewerQGL::endSelection(const QPoint& point)
{
